...
import { prisma } from "@/lib/prisma"; // <-- IMPORT NOMBRADO (no default)
...
export async function POST(req: Request) {
  ...
  try {
    ...
    // Verificar participación del usuario en el grupo (tipado explícito para evitar 'any')
    ...
    const everyoneAccepted = Array.from(allPlayers).every((p) => currentAccepted.has(p));
